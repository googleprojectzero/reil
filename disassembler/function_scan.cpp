...
std::set<uint64_t> FunctionScanAArch64(const MemoryImage& memory_image) {
  ...
  for (auto& mapping : memory_image.mappings()) {
    if (mapping.executable) {
      ...
      for (size_t offset = 0; offset < mapping.data.size(); offset += 4) {
        uint64_t address = mapping.address + offset;
        uint32_t opcode;

        memcpy(&opcode, &mapping.data[offset], sizeof(opcode));

        VLOG(4) << std::hex << address << ": " << opcode << " "
                << aarch64::decoder::DecodeInstruction(address, opcode);
...
std::set<uint64_t> FunctionScan(const MemoryImage& memory_image) {
  if (memory_image.architecture_name() == "aarch64") {
    return FunctionScanAArch64(memory_image);
  } else {
